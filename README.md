...
```sh
npm install git+ssh://git@github.com:stainless-sdks/cellect-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install cellect`
...
<!-- prettier-ignore -->
```ts
const response = await client.healthCheck().catch(async (err) => {
  if (err instanceof Cellect.APIError) {
...
const response = await client.healthCheck().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.healthCheck().withResponse();
...
1. Via the `CELLECT_LOG` environment variable
...
const client = new Cellect({
  logger: logger.child({ name: 'Cellect' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/cellect-typescript/issues) with questions, bugs, or suggestions.
